...
\sidenote{
  \emph{handle\_cast}: A gen\_server callback function
...
, handle\_call%
\sidenote[5]{
  \emph{handle\_call}: A gen\_server callback function used to handle
...
\sidenote[10]{
  ...
  the other types of messages e.g., plain Erlang messages, \abbr{TCP},
  \abbr{UDP} messages and so on.
}
...
commands on different back ends. We implemented back ends in \abbr{ETS}, Redis
...
as \abbr{CPU} time, memory consumed, number of function calls made, number of
messages
...
data structures finally choosing \abbr{ETS} for its performance and added
features.
