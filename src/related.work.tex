...
\begin{itemize}
  ...
  \iterm{Acceptor}: Acceptors form the ``memory'' of the algorithm that allows
  the algorithm to converge to a single value.
  \iterm{Learner}: The chosen values are ``learned'' by the other processes.
\end{itemize}
...
multiple times for mutliple values. We can batch together multiple values
...
Paxos can be optimized to a further by prespecifying the collision
...
Machine (FSM). The idea is that each process is run as an instance of this
FSM, contrary to the regular view of process with a single well defined role.
While this view of Paxos as a FSM is very helpful in the understanding of the
protocol we choose to use the protocol in the aleady established form of a
single role per process.
...
First Out (FIFO) basis. It is one of the notable implementations for leader
election in Erlang.
...
controls, automic access, timestamps and so on.
...
\sidenote[-4]{NoSQL, generally called ``Not Only SQL'' or ``Not Relational'',
  are set of data stores that follow weaker consistency model than ACID and are
characterised by their ability to scale their operations \citep{Cattell10}.}
...
the other requirements to be used as a lock manager, absense of strong
...
featureset of NoSQL data stores, it also provides strong consistency and
atomic counters.
