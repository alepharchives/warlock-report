...
client connections are over \abbr{TCP} and use the Redis binary protocol
\citep{RedisProtocol}%
\sidenote[-2]{
...
least the simple \abbr{API} required for a key value store.
...
\begin{itemize}
  \item L1 initiates the protocol by issuing \code{P1A} message.
  \item L1 receives quorum acceptance and all the acceptors accept its ballot.
  \item L2 initiates the protocol by issuing \code{P1A} message.
  \item L2 receives quorum acceptance and all the acceptors accept its ballot.
  \item L1 initiates \code{P2A} phase only to be preempted with a higher ballot.
  \item L1 initiates phase 1 with a \code{P1A} message.
  \item L2 initiates \code{P2A} phase only to be preempted with a higher ballot.
\end{itemize}
